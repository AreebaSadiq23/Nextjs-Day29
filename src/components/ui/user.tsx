...
const RandomUser: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-300 text-foreground">
      <h1 className="text-3xl font-bold mb-4 text-fuchsia-900">Random User Generator</h1>
      <p className="text-muted-foreground mb-6 text-fuchsia-500">
        Click the button below to fetch a random user profile.
      </p>
      <Button onClick={fetchRandomUser} className="mb-6 bg-lime-300 text-slate-900 hover:bg-emerald-300">
        Fetch New User
      </Button>
      {loading && (
      ...
      {user && (
        <Card className="border-0 shadow-lg rounded-lg overflow-hidden max-w-sm relative bg-red-300">
          <CardHeader className="h-32 bg-red-400 relative">
            <Image
              src={user.image}
              alt={user.name}
              width={80}
          ...
          <CardContent className="p-6 pt-12 text-center">
            ...
            <Button
              variant="outline"
              className="mt-4  bg-lime-300  hover:bg-emerald-300"
